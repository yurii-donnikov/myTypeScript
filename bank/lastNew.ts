  ...
  changeCard(): void {
    ...
    for (let i = 0; i < childrenPopup.length; i++) {
      if (childrenPopup[i].className === 'Debet' || childrenPopup[i].className === 'Credit') {
        ...
        if (this.isFlag) {
          if (this.newClient && !this.newClient.checks) {
            this.newClient.checks = [];
          }
          if (this.newClient) {
            if (this.indexObject) {
              this.newClient.checks[this.indexObject] = {} as Credit | Debet;
              let name: string = childrenPopup[i].className;
              if (childrenPopup[i].className === "Debet") {
                this.newClient.checks[this.indexObject].name = "Debet";
              } else {
                this.newClient.checks[this.indexObject].name = "Credit";
              }
          ...
          if (((childrenPopup[i]) as HTMLInputElement).type === 'radio') {
            ...
            if (this.isFlag) {
              ...
              //   }
              //     if(document.getElementsByClassName(childrenPopup[i].getAttribute('data-id'))){
              // document.getElementsByClassName(childrenPopup[i].getAttribute('data-id'))[this.indexActiveCard].innerHTML = String(((childrenPopup[i]) as HTMLInputElement).checked);
